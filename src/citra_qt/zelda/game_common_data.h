...
#include "citra_qt/zelda/game_items.h"
#include "citra_qt/zelda/game_player.h"
#include "citra_qt/zelda/game_types.h"

namespace zelda::game {

constexpr u16 HhmmToTime(int hours, int minutes, int seconds = 0) {
...
struct CommonData {
    FormEquipmentData& GetFormEquipment() {
        const auto idx = save.player_form == Player::Form::Human ? 0 : u8(save.player_form);
...
inline Ptr<CommonData> GetCommonData() {
    return Ptr<CommonData>{0x7751D8};
}
